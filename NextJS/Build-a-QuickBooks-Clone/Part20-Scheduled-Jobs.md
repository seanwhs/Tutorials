...
Goal of this part: teach Inngest to run functions on a timer rather than in response to an event. We'll build a daily overdue-invoice reminder job (reusing Part 18's AR aging logic) and a simple recurring-invoice generator.
    ...
    for (const org of allOrgs) {
      await step.run(`check-org-${org.id}`, async () => {
        ...
        for (const invoice of overdue) {
          // For this part, we log instead of sending a real email -
          // same intentional simplification as Part 19.
          console.log(
...
Check your Next.js terminal — you should see `[REMINDER]` log lines for any invoices you have that are genuinely overdue (past their due date and still status "sent" or "partially_paid" — go create one with a past due date via `/dashboard/invoices/new` if you don't have one yet, exactly as suggested in Part 18).
...
- description (text, required) — single line item description for simplicity
- amountCents (bigint, not null)
- dayOfMonth (integer, not null) — e.g. 1 for "generate on the 1st of every month"
- isActive (boolean, default true)
- lastGeneratedDate (date, optional) — tracks the last date we generated an invoice from this template, so we don't double-generate if the job runs more than once on the same day
- createdAt (timestamp)
