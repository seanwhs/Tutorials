...
**Goal:** put your app on the real internet, entirely on free tiers — no credit card required anywhere in this process — using Vercel to host the app, and connecting it to your already-free Neon, Clerk, and Inngest accounts. By the end, you'll have a real URL you can share with anyone.
...
**A critical double-check before continuing:** confirm .env.local was never pushed. Look through your files on GitHub — you should NOT see .env.local listed anywhere. If you do see it, stop, remove it, and rotate every secret in it immediately (treat any exposed key as compromised) before proceeding — this is exactly why we relied on .gitignore's default behavior throughout the course.
...
Confirm you completed step 5 (adding your Vercel URL to Clerk's allowed origins/redirect URLs). Also confirm you're using the SAME Clerk application's keys in both places, or if you deliberately created a separate production Clerk app, that you swapped in ITS keys into Vercel's environment variables.

**Database connection errors only under real traffic on Vercel, but everything worked fine locally**
This is the classic pooled-vs-unpooled symptom (step 8) — confirm src/lib/db/index.ts uses `process.env.DATABASE_URL` (the pooled one with `-pooler` in the hostname), not `DATABASE_URL_UNPOOLED`. Using the unpooled/direct connection string for your live running app is the most common cause of "too many connections" errors once real concurrent users show up.

**The Clerk organization.created webhook doesn't fire when you create a new org on the live site**
Double check the endpoint URL you entered in Clerk's dashboard exactly matches your real deployed domain plus `/api/webhooks/clerk` (typos here are common — copy-paste the URL from your browser's address bar rather than typing it by hand). Also confirm CLERK_WEBHOOK_SECRET is set in Vercel's environment variables and that you redeployed AFTER adding it (environment variable changes require a new deployment to take effect).

**Webhook endpoint returns a 400 or signature verification error**
This usually means the CLERK_WEBHOOK_SECRET in Vercel doesn't match the one shown in Clerk's dashboard for that specific endpoint — each webhook endpoint gets its own unique signing secret; if you have multiple endpoints configured, make sure you copied the secret for the correct one.

**Inngest Cloud can't find your functions when you try to sync production**
Confirm the URL you entered in Inngest's dashboard is your real, currently-deployed Vercel URL (not localhost, and not an old preview deployment URL that may have expired) plus `/api/inngest` exactly. Visit that URL directly in your browser first — it should respond, proving the route is genuinely live before asking Inngest to sync with it.

**Migrations don't seem to have applied to the production database**
Confirm you genuinely pointed your local `npm run db:migrate` command at your PRODUCTION database's DATABASE_URL_UNPOOLED (via a temporarily swapped .env.local or a separate .env.production.local file) rather than accidentally running it against your local development database again.

**"git push" doesn't trigger a new Vercel deployment**
Confirm your Vercel project is actually connected to the correct GitHub repository and branch (check Project Settings -> Git in Vercel's dashboard). If you renamed your default branch or pushed to a different branch than the one Vercel is watching, deployments won't trigger automatically.

**You accidentally committed and pushed .env.local to GitHub**
Treat every secret in that file as compromised immediately: rotate your Clerk secret key, generate a new Neon password/connection string, and any other exposed key, in each service's own dashboard. Then remove the file from your repository's history — but for a fully public repo where the secret may have already been scraped, rotating the actual credentials is the only real fix; simply deleting the file from a future commit does not undo an already-public exposure.

---

### What's next

Part 24: What's Next — the final part of this course. A roadmap of everything genuinely still ahead per our original project plan (Phase 2: Plaid, Stripe payments, recurring billing, roles/permissions, reconciliation; Phase 3: payroll, tax forms, inventory) — what to tackle first, and how to keep growing this project well beyond this course.
