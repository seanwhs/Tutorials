...
**Prerequisite:** Parts 1-4 completed.
...
Check your browser console (F12 or right-click -> Inspect -> Console tab) for an error message. Usually caused by an outdated `@clerk/nextjs` version — run `npm install @clerk/nextjs@latest` and restart the dev server.
...
This usually means you're not actually signed in — confirm `<SignedIn>` content (the switcher, avatar) is visible on `/` first. If you only see `<SignedOut>` content, sign in again from Part 4's sign-in button.
...
Ready for **Part 6: Getting a Free Database with Neon** whenever?
